...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='forgeflow-adherence-') as tmpdir:
            ...
            scenarios.append(_positive_case('large-migration-task', fixtures_root / 'large-migration-task', 'high', 'long-run', workspace))
            scenarios.append(_positive_case('large-approved-but-unsafe', fixtures_root / 'large-approved-but-unsafe', 'high', 'long-run', workspace))
            ...
            scenarios.append(_negative_advance_case('missing-run-state-before-spec-review', negative_root / 'missing-run-state-before-spec-review', 'high', 'execute', 'missing required artifacts for spec-review: run-state', workspace))
            scenarios.append(_negative_run_case('missing-eval-record-before-long-run', negative_root / 'missing-eval-record-before-long-run', 'high', 'long-run requires artifacts satisfying gate worth_long_run_capture: eval-record', workspace))
            ...
            scenarios.append(_negative_run_case('large-spec-quality-mismatch', negative_root / 'large-spec-quality-mismatch', 'high', 'spec-review requires approved spec review-report artifact', workspace))
            scenarios.append(_negative_run_case('large-session-state-stale-review-ref', negative_root / 'large-session-state-stale-review-ref', 'high', 'checkpoint.json latest_review_ref review-report-stale.json does not exist', workspace))
