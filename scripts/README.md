...
`make validate` is the deterministic validation entry point. It runs `check-env`, repo validators, focused pytest contracts, and static plugin smoke matrix checks in order. Use `make check-env` only when you intentionally want the narrower environment diagnostic.
