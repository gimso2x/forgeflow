...
- `run_first_demo.py` : disposable workspace에서 첫 실행 demo task를 init/status까지 실행
...
- `make validate-structure` — version drift, generated adapters, schemas, policy, context paths, skill contracts, hooks.
...
Use `make check-env` only when you intentionally want the narrower environment diagnostic.
...
Checkout 없이 PowerShell에서 bootstrap할 때는 `irm ... | python - --dry-run`으로 먼저 확인한 뒤 `irm ... | python - --force`를 사용한다. `python` 대신 Windows launcher만 있는 환경에서는 checkout 안의 `install_codex_plugin.ps1` wrapper를 사용한다.
...
- `demo`는 `scripts/run_first_demo.py`로 disposable workspace에 `demo-readme` task를 만들고 `status`까지 출력한다.
