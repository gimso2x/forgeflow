...
checks = {
    ...
    'skills/ff-review/SKILL.md': ['docs/review-runtime-contract.md', 'brief / evidence / scope / constraints', 'read-only except for review artifacts', 'observed evidence', 'Cross-role conflicts', 'Evidence Source', 'Evidence Level', 'observed | reported | missing', 'lead/member guardrails', 'does not spawn unmanaged child work', 'Adapter compliance checklist', 'source classified', 'fetch reproduced', 'fetch_ledger_complete', 'fetch posture constrained', 'normalization complete', 'limitations visible', 'review ownership delegated', 'Checklist Version', 'Review tool posture', 'hand it back to execute', 'role-pass record', 'chat-only role completion claims', 'role routing rationale', 'Active roles', 'Skipped roles', 'silently broadening or narrowing review scope', 'role=<reviewer> scope=<artifact section/evidence IDs> at=<ISO8601>', 'role evidence map', 'stable evidence IDs', 'unique within `normalized-input.md`', 'Role trigger matrix', 'missing trigger evidence', '.diff` / `.patch`', 'Evidence Escalation Log', 'hidden adapter state', 'role input packet', 'Role model hints', 'role capability hints', 'strongest reasoning available', 'Model choice must never change role routing', 'aggregation_owner` must match the single `lead_reviewer`', 'routing consistency check', 'constraints.roles', 'packet freshness', 'stale packets block judgment', 'scope_source_map', 'Evidence Gap Register', 'evidence integrity check', 'source, type, fetch status, fetched_at/run label, freshness status, and evidence level', 'Standalone Input Source (type, original input, fetch status, fetched_at/run label, freshness status, access posture, mutation check, Fetch Method Ledger references, Evidence Source Map references, Adapter Handoff Checklist statuses)', 'claim marker', 'read back from the artifact', 'Access posture and mutation check', 'Fetch Method Ledger'],
    'skills/ff-review/references/role-checklists.md': ['Checklist version:', 'role input packet', 'Role trigger matrix', 'role evidence map', 'scoped files/ranges/exclusions', 'constraints/focus flags', 'criteria basis', 'Role criteria are not interchangeable', 'Evidence Escalation Log', 'blocked: missing role input packet'],
    ...
    'skills/_shared/context-resume.md': ['Checkpoint-first', 'Handoff Boundary', 'requested/forbidden action', 'Minimum Read Set', 'Reader Summary', 'Evidence Index', 'Auto-chain resume'],
    'templates/brief.md': ['Route Rationale', 'Budget Note', 'Suggested Next Skill', 'Suggested specialists'],
    'templates/review-report.md': ['Evidence Source', 'Evidence Level', 'observed | reported | missing', 'Checklist Version', 'role-pass record', 'Chat-only completion claims are not evidence', 'Normalization Gate', 'Fetched At', 'Freshness Status', 'Access Posture', 'Mutation Check', 'Fetch Method Ledger', 'Evidence Source Map', 'Adapter Handoff Checklist', 'fetch_ledger_complete', 'canonical_review_ownership', 'Role routing rationale', 'Role evidence map', 'Evidence freshness', 'Evidence Freshness', 'Role criteria basis summary', 'Criteria Basis Used', 'Evidence gap register', 'Review ownership plan', 'exactly one lead reviewer', 'Conflict policy', 'Human Review Gate', 'Active roles', 'Skipped roles', 'Trigger Rationale', 'Claim Marker', 'Independence Check', 'Evidence Escalation Log', 'Requester Role', 'Approval Impact', 'packet_freshness'],
    'templates/normalized-input.md': ['brief_present', 'evidence_present_or_blocked', 'scope_explicit', 'constraints_explicit', 'limitations_visible', 'role evidence map', 'stable evidence IDs', 'unique within this file', 'fetch_status', 'fetched_at', 'freshness_status', 'current | stale | unknown', 'limitations', 'Evidence Gap Register', 'missing | partial | sampled | blocked', 'ignored_flags', 'scope_source_map', 'Role trigger matrix', 'run | skipped | blocked', 'evidence integrity check', 'id_resolution', 'source_map_match', 'freshness_match', 'scope_map_coverage', 'role_map_coverage', 'packet_reference_freshness', 'role input packet readiness', 'READY | BLOCKED | SKIPPED', 'role input packets', 'trigger=<matrix row>', 'evidence_ids=<role evidence map IDs>', 'criteria_basis', 'packet_freshness', 'role capability hints', 'provider-specific requirements', 'review ownership plan', 'lead_reviewer', 'exactly one identifier', 'member_assignments', 'claim_marker_integrity', 'aggregation_owner', 'child_work_policy', 'conflict_policy', 'product_mutation_policy', 'adapter handoff checklist', 'source_classified', 'fetch_reproduced', 'fetch_ledger_complete', 'fetch_posture_constrained', 'normalization_complete', 'canonical_review_ownership', 'Consistency guard', 'constraints.roles'],
    'templates/plan.md': ['Execution Pattern', 'Applied Evolution Rules'],
    'templates/ledger.md': ['Claim Marker', 'role=<worker|specialist|spec-reviewer|quality-reviewer> scope=<repo paths or artifact section> at=<ISO8601>', 'not chat-only claims', 'atomic at the artifact level', 're-read this task row'],
    'docs/advisory-guidelines.md': ['Route Budget Guide', 'small:', 'medium:', 'high:', 'epic:', 'Non-goals'],
    'docs/review-runtime-contract.md': ['Adapter-neutral core', 'Thin adapter responsibilities', 'Adapter compliance checklist', 'Source classified', 'Fetch reproduced', 'Fetch ledger complete', '`fetch_ledger_complete`', 'Fetch posture constrained', 'Normalization complete', 'Limitations visible', 'Review ownership delegated', 'Role separation', 'Stage tool catalog', 'Evidence levels', 'Human review gate', 'Minimal team-mode absorption', 'Lead/member guardrails', 'Claim marker', 'Ownership plan', 'conflict policy', 'role=<reviewer> scope=<artifact section/evidence IDs> at=<ISO8601>', 'Non-goal', 'input-source.md', 'normalized-input.md', 'source classification rationale', 'role-pass record', 'Active roles', 'Skipped roles', 'Chat-only role completion claims', 'role evidence map', 'stable, unique evidence ID', 'fetch_status', 'Evidence Gap Register', 'ignored_flags', 'scope_source_map', 'Evidence Source Map', 'evidence integrity check', 'source, type, fetch status, fetched_at/run label, freshness status, and evidence level', 'Standalone Input Source summary', 'Adapter Handoff Checklist statuses', 'stale or unknown evidence posture', 'role trigger matrix', 'missing trigger evidence', '.diff` / `.patch` file path', 'Independence Check', 'Evidence escalation log', 'hidden adapter state', 'role input packet', 'role input packets', 'role capability hints', 'Role model hints', 'strongest reasoning available', 'Model choice must not change role routing', 'Role routing consistency', 'constraints.roles', 'packet freshness', 'claim marker integrity', 'read back from the artifact', 'access posture', 'Fetch Method Ledger', 'multi-fetch ledger rows back cited evidence'],
    'README.md': ['docs/review-runtime-contract.md', 'adapter-neutral input normalization', 'input-source.md', 'normalized-input.md', 'source classification rationale', 'role trigger matrix', 'review ownership plan', 'conflict policy', 'ignored flags', 'adapter별 별도 report나 자동 승인 경로 없음', '.diff`/`.patch` 파일', 'lead reviewer', 'member reviewer', 'unmanaged child work', 'role=<reviewer> scope=<artifact section/evidence IDs> at=<ISO8601>', 'Multi-harness 원칙', 'adapter-neutral core contract', 'hidden provider state', 'Evidence Escalation Log', 'role input packet', 'role capability hints', 'constraints.roles', 'packet freshness', 'fetch posture constrained', 'fetch_ledger_complete', '`fetched_at`', '`freshness_status`', 'access posture', 'mutation check', 'read-only/verification-only posture', 'Fetch Method Ledger', 'Evidence Source Map과 type/fetch status/`fetched_at`/`freshness_status`/evidence level', 'Adapter Handoff Checklist', 'standalone input source summary'],
    'docs/adapter-config.md': ['Multi-harness routing invariants', 'Canonical stage contract first', 'Harness-specific code paths stay shallow', 'Artifact handoff is the boundary', 'Review adapters normalize before judging', 'Review fetch ledger resolves before handoff', 'fetch_ledger_complete', 'Role packet handoff is explicit', 'constraints.roles', 'Role trigger matrix', 'role evidence map', 'role input packet readiness', 'role input packets', 'hidden provider state', 'Validation follows touched surface'],
}
failures = []
...
adapter_handoff_fields = [
    'source_classified',
    'fetch_reproduced',
    'fetch_ledger_complete',
    'fetch_posture_constrained',
...
evidence_integrity_pos = normalized_input_template.find('## evidence integrity check')
...
role_capability_hints_pos = normalized_input_template.find('## role capability hints')
...
if not (trigger_matrix_pos != -1 and role_evidence_map_pos != -1 and evidence_integrity_pos != -1 and role_packet_readiness_pos != -1 and role_packets_pos != -1 and role_capability_hints_pos != -1 and ownership_plan_pos != -1 and normalization_gate_pos != -1 and adapter_handoff_pos != -1 and trigger_matrix_pos < role_evidence_map_pos < evidence_integrity_pos < role_packet_readiness_pos < role_packets_pos < role_capability_hints_pos < ownership_plan_pos < normalization_gate_pos < adapter_handoff_pos):
    failures.append('templates/normalized-input.md: role trigger matrix must appear before role evidence map, evidence integrity check, role input packet readiness, role input packets, role capability hints, review ownership plan, normalization gate, and adapter handoff checklist')
...
role_evidence_section = section_between(normalized_input_template, '## role evidence map', '## evidence integrity check')
evidence_integrity_section = section_between(normalized_input_template, '## evidence integrity check', '## role input packet readiness')
role_packet_section = section_between(normalized_input_template, '## role input packet readiness', '## role input packets')
role_packets_section = section_between(normalized_input_template, '## role input packets', '## role capability hints')
role_capability_hints_section = section_between(normalized_input_template, '## role capability hints', '## review ownership plan')
for role in role_trigger_roles:
    ...
    if role_marker not in role_capability_hints_section:
        failures.append(f'templates/normalized-input.md: role capability hints must include {role}')
...
consistency_fragments = ['Consistency guard', 'constraints.roles', 'Role trigger matrix', 'blocked rationale']
missing_consistency_fragments = [fragment for fragment in consistency_fragments if fragment not in role_evidence_section]
if missing_consistency_fragments:
    failures.append(f'templates/normalized-input.md: role evidence map must preserve routing consistency guard {missing_consistency_fragments}')
integrity_required_fragments = ['id_resolution', 'source_map_match', 'freshness_match', 'scope_map_coverage', 'role_map_coverage', 'packet_reference_freshness', 'blocking_notes', 'Evidence Source Map']
missing_integrity_fragments = [fragment for fragment in integrity_required_fragments if fragment not in evidence_integrity_section]
if missing_integrity_fragments:
    failures.append(f'templates/normalized-input.md: evidence integrity check must preserve cross-artifact evidence ID validation {missing_integrity_fragments}')
packet_required_fragments = ['READY | BLOCKED | SKIPPED', 'trigger,evidence_ids,scope,constraints,criteria_basis,limitations,packet_freshness']
...
packet_row_fragments = ['trigger=<matrix row>', 'evidence_ids=<role evidence map IDs>', 'scope=<files/ranges/exclusions>', 'constraints=<roles/focus/user_rules/inferred_rules/ignored_flags>', 'limitations=<evidence limitations/truncation/fetch failures>', 'packet_freshness=<current after latest evidence/scope/constraint/routing change>']
...
capability_hint_fragments = ['Optional, advisory only', 'provider-specific requirements', 'must not affect routing', 'human review gate']
missing_capability_hint_fragments = [fragment for fragment in capability_hint_fragments if fragment not in role_capability_hints_section]
if missing_capability_hint_fragments:
    failures.append(f'templates/normalized-input.md: role capability hints must stay advisory and semantics-neutral {missing_capability_hint_fragments}')
ownership_section = section_between(normalized_input_template, '## review ownership plan', '## normalization gate')
ownership_required_fragments = ['exactly one lead', 'exactly one identifier', 'aggregation_owner', 'claim_marker_integrity', 'unique | duplicate | unreadable | not_applicable', 'read back from the artifact', 'conflict_policy', 'Human Review Gate', 'does not silently choose a winner']
missing_ownership_fragments = [fragment for fragment in ownership_required_fragments if fragment not in ownership_section]
if missing_ownership_fragments:
    failures.append(f'templates/normalized-input.md: review ownership plan must preserve single-lead aggregation guardrails {missing_ownership_fragments}')
review_ownership_line = next((line for line in review_template.splitlines() if 'Review ownership plan:' in line), '')
