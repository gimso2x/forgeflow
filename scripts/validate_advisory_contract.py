...
checks = {
    ...
    'templates/review-report.md': ['Evidence Source', 'Evidence Level', 'observed | reported | missing', 'Checklist Version', 'role-pass record', 'Chat-only completion claims are not evidence', 'Normalization Gate', 'Role routing rationale', 'Role evidence map', 'Review ownership plan', 'exactly one lead reviewer', 'Active roles', 'Skipped roles', 'Trigger Rationale', 'Claim Marker', 'Independence Check', 'Evidence Escalation Log', 'Requester Role', 'Approval Impact', 'packet_freshness'],
    'templates/normalized-input.md': ['brief_present', 'evidence_present_or_blocked', 'scope_explicit', 'constraints_explicit', 'limitations_visible', 'role evidence map', 'stable evidence IDs', 'unique within this file', 'fetch_status', 'limitations', 'ignored_flags', 'scope_source_map', 'Role trigger matrix', 'run | skipped | blocked', 'role input packet readiness', 'READY | BLOCKED | SKIPPED', 'role input packets', 'trigger=<matrix row>', 'evidence_ids=<role evidence map IDs>', 'packet_freshness', 'role capability hints', 'provider-specific requirements', 'review ownership plan', 'lead_reviewer', 'exactly one identifier', 'member_assignments', 'aggregation_owner', 'child_work_policy', 'product_mutation_policy', 'adapter handoff checklist', 'source_classified', 'fetch_reproduced', 'normalization_complete', 'canonical_review_ownership', 'Consistency guard', 'constraints.roles'],
...
ownership_required_fragments = ['exactly one lead', 'exactly one identifier', 'aggregation_owner']
