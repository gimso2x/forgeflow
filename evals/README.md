...
- autonomous maintainer final reports keep the required Korean headings (`요약`, `변경한 것`, `검증`, `커밋/푸시`, `다음 후보`, `블로커`), exact validation evidence, commit/push status, no separate `send_message` delivery step, no live provider/plugin E2E overclaim, explicit no-op commit refusal when no safe focused change is available, and failed pushes reported as blockers rather than delivered changes
