...
Codex plugin marketplace 등록, plugin 설치, slash command 실행은 **대상 프로젝트 루트** 또는 Codex App이 사용하는 WSL 환경에서 수행합니다. ForgeFlow checkout이나 Codex plugin cache 안에서 workflow command를 실행하면 `.forgeflow/tasks/<task-id>/` 산출물이 잘못된 위치에 생길 수 있습니다.
