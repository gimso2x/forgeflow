...
ForgeFlow의 core contract는 `skills/`, `templates/`, `docs/review-runtime-contract.md`에 있고, Claude Code/Codex/Cursor는 얇은 harness wrapper로만 동작합니다. 새 어댑터나 기존 어댑터별 예외를 추가할 때는 먼저 이 불변식을 확인합니다.
...
| Slash 형식 | 콜론 없음: `/clarify`, `/ff-plan`, `/execute`, `/ff-review`, `/ship`, `/long-run`, `/benchmark`, `/ff-config` (Claude/Codex는 `/forgeflow:clarify`) |
...
|--------|---------------------|------------------|-----------|
| Claude Code | ~64s | ~243s | 1.0x (기준) |
| Codex CLI | ~119s | ~148s | 1.8-1.5x |
| Cursor | (IDE-bound, 벤치마크 미포함) | — | — |

### 타임아웃 가이드

| 작업 규모 | Claude | Codex | Cursor | 안전 상한 |
|----------|--------|-------|--------|-----------|
| small | 120s | 180s | 180s | 300s |
| medium | 360s | 240s | 360s | 600s |
| high | 600s | 480s | 600s | 900s |
| epic | 1200s | 900s | 1200s | 1800s |
