...
**작업 위치 원칙:** plugin/extension 설치·cache 위치와 실제 workflow 대상 프로젝트 루트를 분리합니다. `/forgeflow:clarify`, `/forgeflow:execute` 같은 stage 명령은 변경하려는 프로젝트 루트에서 실행하고, 산출물은 기본적으로 글로벌 프로젝트 저장소 `~/.forgeflow/projects/<project-slug>/tasks/<task-id>/` 아래에 기록해야 합니다. 레포-local `.forgeflow/tasks/<task-id>/`는 명시 local storage일 때만 씁니다. 어댑터 cache 안에서 실행 중이면 `--task-dir ~/.forgeflow/projects/<project-slug>/tasks/<task-id>`처럼 명시 경로를 사용합니다.
...
Gemini extension 설치·업데이트는 extension manager/cache에서 처리될 수 있지만, `/forgeflow:clarify`, `/forgeflow:execute` 같은 workflow 명령은 **대상 프로젝트 루트**에서 실행합니다. Disposable 또는 untrusted repo에서 headless smoke를 실행할 때는 `--skip-trust`를 함께 사용하고, cache 컨텍스트에서 열렸다면 `--task-dir ~/.forgeflow/projects/<project-slug>/tasks/<task-id>`로 산출물 위치를 명시합니다.
...
| 산출물 경로 | 기본 `~/.forgeflow/projects/<project-slug>/tasks/<task-id>/`; local storage를 명시한 경우만 `<workspace>/.forgeflow/tasks/<task-id>/` (plugin cache에 쓰지 않음) |
...
프로젝트별 ForgeFlow storage root의 `defaults.md`를 두면 모든 태스크에 적용할 기본값을 설정할 수 있습니다. 기본 storage root는 `~/.forgeflow/projects/<project-slug>/`입니다. 레포 안에 남기는 것은 opt-in bootstrap 용도(`.forgeflow/defaults.md`에 `storage.mode: local` 등)만 허용합니다.
...
| `storage.mode` | `global` | `global`이면 `~/.forgeflow/projects/<project-slug>/`, `local`이면 `<repo>/.forgeflow/` 사용 |
| `storage.root` | `~/.forgeflow` | global 모드의 홈 디렉터리 override. env `FORGEFLOW_HOME`이 더 우선 |
