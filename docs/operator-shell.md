...
`run_runtime_sample.py` is the safe demo entry. It copies a fixture to a disposable workspace before running, so tracked fixture artifacts do not get dirtied by a sample command. In a fresh clone, use the repo-managed Make target so the command runs through the local `.venv` created by setup.
