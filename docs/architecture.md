...
ForgeFlow is an artifact-first workflow contract plus a lightweight enforcement runtime for Claude Code, Codex, and Gemini CLI. The runtime is a zero-dependency Python library (`forgeflow_runtime/`) that provides staged workflows, gate enforcement, evidence trails, and optional multi-agent orchestration.
