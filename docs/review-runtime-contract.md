...
`review-report.md` remains the single review result artifact. In standalone mode its Standalone Input Source summary must echo the fetch status plus fetched_at/run label, freshness status, access posture, mutation check, Fetch Method Ledger references, and Evidence Source Map references from `input-source.md`/`normalized-input.md`, so readers can see stale or unknown evidence posture, whether fetches stayed read-only/verification-only, and which multi-fetch ledger rows back cited evidence without reopening the normalization artifacts. Adapter-specific reports are not allowed.
