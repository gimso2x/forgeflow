...
- `review ownership plan`: the lead reviewer, any member assignments, aggregation owner, child-work policy, conflict policy, and product-mutation policy recorded before delegated or parallel reviewer passes start
...
- **Ownership plan**: standalone normalization records the lead reviewer, member assignments, aggregation owner, no-child-work policy, conflict policy, and product-mutation policy in `normalized-input.md` before any delegated/member pass starts. The report cites that plan so aggregation can be audited without chat logs and unresolved disagreements stay visible for the human gate.
