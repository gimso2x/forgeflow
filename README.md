...
현재 릴리즈: **v0.13.0**
CHANGELOG 최신 항목도 v0.13.0 기준입니다.
...
make test       # 일반 개발: 전체 테스트
make validate   # 릴리즈 전: 구조/정책/adapter/eval 전체 검증
```

`make ci`는 `check-env → validate → test`를 순서대로 실행합니다. 상세 타겟은 [Makefile](Makefile.md)과 [INSTALL.md](INSTALL.md)을 참고하세요.
