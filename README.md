...
```text
/forgeflow:config auto true  → auto 모드 기본 활성화 (선택)
...
```text
/forgeflow:config auto true
```

이 명령 하나로 프로젝트에 `.forgeflow/defaults.md`가 생성되고, 이후 모든 태스크가 자동 체이닝으로 실행됩니다.

```text
/forgeflow:config            # 현재 설정 확인
/forgeflow:config auto false # auto 끄기
```
