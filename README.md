...
Codex App은 Codex CLI와 같은 plugin/skill 생태계를 공유합니다. Windows에서 WSL backend를 쓰면 WSL 안의 Codex plugin marketplace 설정을 읽고, native Windows backend를 쓰면 Windows 쪽 Codex 설정을 읽습니다. 설치나 enable 상태를 바꾼 뒤에는 Codex App을 새로 시작하세요.
...
Codex App에서 확인할 때는 작은 문서 수정 같은 저위험 작업으로 먼저 smoke를 돌리세요. 목표 프로젝트 루트에서 `/forgeflow:clarify --auto <작업>`를 실행하고, 산출물이 `~/.forgeflow/projects/<project-slug>/tasks/<task-id>/`에 생기는지, plugin/cache 디렉터리 안에 쓰이지 않는지 확인합니다.
...
**Codex App + WSL:** WSL backend를 선택했다면 WSL 안의 `codex plugin marketplace add /path/to/forgeflow`와 `codex plugin add forgeflow@forgeflow`가 기준입니다. Windows native backend를 선택했다면 Windows shell에서 같은 설치 확인을 수행하세요.
...
**벤치마킹에서 반영한 원칙:** ForgeFlow는 Superpowers의 "evidence before completion"과 subagent review loop, Ouroboros의 specification-first ledger/replay 관점을 참고하되 별도 런타임을 추가하지 않습니다. 대신 `brief.md` Goal Contract, `ledger.md`, `checkpoint.md`, route-aware `--auto`, 그리고 `review-report.md` evidence level로 같은 효과를 Markdown contract 안에서 냅니다. 참고: [Superpowers](https://github.com/obra/superpowers), [Ouroboros](https://github.com/Q00/ouroboros), [Codex plugins](https://help.openai.com/en/articles/20001256-plugins-in-codex).
...
| Route  | Stages                                                                                    | When                       |
| ------ | ----------------------------------------------------------------------------------------- | -------------------------- |
| small  | clarify → execute → ship (self-verify fast path)                                  | 저위험, 소규모, 쉬운 롤백  |
...
`project-slug`는 기본적으로 `basename(repo_root)`이고, 같은 이름의 체크아웃이 충돌하면 절대 경로 해시를 붙입니다(예: `forgeflow-a13f9c`). `tasks/` 앞에 프로젝트 폴더가 있어야 여러 레포 작업이 섞이지 않습니다.
...
- **small:** clarify → execute → ship (execute의 self-verify가 formal review를 대체)
...
- `--real` 외부 실행 안전 확인
- ship 브랜치 처리 선택 (비-auto일 때 merge/PR/keep/discard; auto에서는 기본 Merge locally, 충돌/파괴적 동작은 auto-break)
- discard 최종 확인
- ship 품질 루프백
...
여기서 `SKILL.md`는 루트 marketplace summary입니다. `skills/*/SKILL.md` 같은 public skill의 Per-skill frontmatter `version`은 skill schema version이며 릴리즈 `VERSION`과 별개이므로 release sync 대상이 아닙니다.
