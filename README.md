...
- `normalized-input.md`: `brief / evidence / scope / constraints` 4-field 구조로 정규화하고, stable evidence IDs, per-item fetch status/limitations, role trigger matrix, role evidence map, review ownership plan으로 각 리뷰어 역할의 실행/스킵 근거, 인용 가능한 증거, lead/member 소유권을 고정
