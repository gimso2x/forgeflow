...
- `scripts/forgeflow_evolution.py inspect` / `list` / `adopt` / `dry-run --rule <id>` / gated `execute --rule <id>` / `retire --rule <id> --reason <why>` / `restore --rule <id> --reason <why>` / `doctor` / `effectiveness --rule <id>` / `promotion-plan --rule <id> [--write]` / `proposal-review --proposal <path>` / `proposal-approve --proposal <path> --approval <approval> --approver <name> --reason <why>` / `proposal-approvals --proposal <path>` / `audit` — evolution policy/runtime-safety summaries, explicitly acknowledged project-local rule execution, lifecycle retirement/restoration, read-only health/effectiveness/promotion-planning checks, optional proposal persistence/review, append-only approval ledger, and audit inspection
