...
`/forgeflow:ff-review`는 파이프라인 없이도 PR, diff, 디렉터리, 기존 artifact를 검토할 수 있습니다.
...
지원 입력은 URL, GitHub PR/commit/compare, repo path, git range, `.diff`/`.patch` 파일, file bundle, existing artifact, ambiguous 입력입니다.
