...
매번 `--auto`를 입력하지 않으려면 두 가지 방법이 있습니다:

1. **brief.md에 설정** — clarify가 작성하는 brief에 `auto: true`를 적으면 해당 태스크는 자동 체이닝
2. **자연어 지시** — `"auto로 진행해"`, `"모든 단계 자동으로 해"` 등 말하면 `--auto`와 동일

프로젝트에서 항상 auto 모드를 사용하려면 `brief.md` 템플릿의 기본값을 `true`로 변경하세요.
