...
Claude Code plugin으로 설치하면 아래 slash skill을 바로 쓴다:

1. `/clarify`
2. `/specify` when requirements need to be formalized
3. `/plan`
4. `/run`
5. `/review`
6. `/ship`

`/forgeflow`는 전체 workflow 설명/입구이고, 실제 진행은 `/clarify`, `/plan`, `/run`, `/review`, `/ship`처럼 단계별로 쓰는 게 맞다. README가 그렇게 읽히는데 명령이 안 되면 그건 제품이 사용자를 속이는 꼴이다. 그래서 단계별 slash skill을 제공한다.
...
`/forgeflow`는 전체 설명용 입구입니다. 실제 workflow는 `/clarify`부터 타세요.
