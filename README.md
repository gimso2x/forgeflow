...
`/forgeflow`는 전체 workflow 설명/입구이고, 실제 진행은 `/forgeflow:clarify`, `/forgeflow:plan`, `/forgeflow:run`, `/forgeflow:review`, `/forgeflow:ship`처럼 단계별로 쓰는 게 맞다. 짧은 `/review`나 `/ship`은 gstack 같은 다른 Claude plugin skill과 충돌할 수 있으니 운영 문서에서는 namespaced form을 정본으로 둔다.
...
- `scripts/forgeflow_evolution.py inspect` / `list` / `adopt` / `dry-run --rule <id>` / gated `execute --rule <id>` / `retire --rule <id> --reason <why>` / `restore --rule <id> --reason <why>` / `doctor` / `effectiveness --rule <id>` / `promotion-plan --rule <id> [--write]` / `proposal-review --proposal <path>` / `proposal-approve --proposal <path> --approval <approval> --approver <name> --reason <why>` / `proposal-approvals --proposal <path>` / `promotion-gate --proposal <path>` / `promotion-decision --proposal <path> --decision approve_policy_gate --decider <name> --reason <why> [--write]` / `audit` — evolution policy/runtime-safety summaries, explicitly acknowledged project-local rule execution, lifecycle retirement/restoration, read-only health/effectiveness/promotion-planning checks, optional proposal persistence/review, append-only approval ledger, and audit inspection
