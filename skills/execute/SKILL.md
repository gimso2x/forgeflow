...
For **high/epic** routes, `/forgeflow:execute --subagent-per-task` runs every plan step through implementer → spec micro-review → quality micro-review subagents. Default remains controller-led.

→ Full loop procedure, parallelism rules, and model hints: [`references/subagent-loop.md`](references/subagent-loop.md)
...
Dispatch templates: `references/implementer-prompt.md`, `references/spec-reviewer-prompt.md`, `references/quality-reviewer-prompt.md`. See [`references/subagent-loop.md`](references/subagent-loop.md) for role mapping.
...
> Controller-led delegation for independent plan steps. Distinct from `--subagent-per-task` mode.

When `brief.md` includes required specialists and the task scope justifies delegation, use the Agent tool to spawn specialist sub-agents. Base prompt: `references/implementer-prompt.md`.

→ When/how to delegate, result handling, parallel delegation: [`references/agent-delegation.md`](references/agent-delegation.md)
