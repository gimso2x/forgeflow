...
Then fill **role input packet readiness** for every active, blocked, or skipped reviewer role. Mark a role `READY` only when its trigger decision, allowed evidence IDs, scoped files/ranges/exclusions, constraints/focus flags, and visible limitations are all present in `normalized-input.md`. Mark it `BLOCKED` when any packet field is missing and prevent that role from judging until the missing field is normalized or recorded as unavailable. Mark skipped roles `SKIPPED` with an explicit non-trigger reason.
