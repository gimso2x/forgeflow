...
- Write only under the current project workspace or the active task directory. Never write inside the plugin installation directory, marketplace cache, extension cache (including `.claude/plugins/cache`, `.codex/plugins`, `.cursor/plugins`, or `~/.cursor/plugins/local`), or `skills/<skill>/`.
...
- Never claim provider/plugin E2E, live CLI smoke, or real Claude/Codex/Cursor behavior unless the exact provider command was actually run in the current task and its result is cited. If only repo validators or deterministic smoke fixtures ran, say that explicitly.
