...
- **Project-local `.forgeflow/` creation is forbidden.** Never write any file or directory under `<repo>/.forgeflow/` for any reason — no tasks, no telemetry, no evolution rules, no benchmarks. The only `.forgeflow/` allowed inside a worktree is the compatibility symlink directory under `<storage-root>/worktrees/<task-id>/.forgeflow/` (see `_shared/isolation.md`). If you find yourself constructing a path like `<repo>/.forgeflow/...`, stop and resolve the correct global path via `forgeflow_storage.py` instead.
...
- Write only under the current project workspace or the active task directory. Never write inside the plugin installation directory, marketplace cache, extension cache (including `.claude/plugins/cache`, `.codex/plugins`, `.cursor/plugins`, `~/.cursor/plugins/local`, or `~/.gemini/antigravity-cli/plugins`), or `skills/<skill>/`.
...
- Never claim provider/plugin E2E, live CLI smoke, or real Claude/Codex/Antigravity CLI behavior unless the exact provider command was actually run in the current task and its result is cited. If only repo validators or deterministic smoke fixtures ran, say that explicitly.
...
- When artifacts are mentioned without an explicit path, assume the resolved project-scoped storage path (`~/.forgeflow/projects/<project-slug>/tasks/<task-id>/` by default), not chat-only fallback.
...
resolved telemetry directory (`~/.forgeflow/projects/<project-slug>/telemetry/` by default) directory.

### Automatic collection via scripts

Run `python3 scripts/telemetry_collect.py` after any stage completes to scan
`<task-dir>` artifacts and append structured events to
`<telemetry-dir>/<task-id>.md`. Run `python3 scripts/telemetry_aggregate.py`
to generate `~/.forgeflow/projects/<project-slug>/telemetry/summary.md` (default resolved telemetry summary) with aggregated metrics.
...
block to `<telemetry-dir>/<task-id>.md`. **반드시 다음 형식을 준수한다**:
...
### Rules
- Create resolved telemetry directory (`~/.forgeflow/projects/<project-slug>/telemetry/` by default) if it does not exist.
- Use one file per task: `<telemetry-dir>/<task-id>.md`.
