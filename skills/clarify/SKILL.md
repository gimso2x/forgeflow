...
10. **Read project defaults** before producing the brief. Read `.forgeflow/defaults.md` if it exists. Propagate settings to `brief.md` frontmatter:
...
13. **Worktree isolation** (medium/high/epic only, unless `--no-isolation` or `defaults.md` `isolation: false`):
