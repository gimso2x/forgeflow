...
- recommended_location: ./GEMINI.md
- Copy this generated adapter into `./GEMINI.md` when wiring ForgeFlow into gemini.

## Installation steps
1. Install as a Gemini CLI extension with `gemini extensions install https://github.com/gimso2x/forgeflow`, or link a checkout during development with `gemini extensions link /path/to/forgeflow`.
2. Update later with `gemini extensions update forgeflow`.
3. Restart Gemini CLI after install/update so extension context is reloaded.
4. Optionally add project-local role presets with `python3 scripts/install_agent_presets.py --adapter gemini --target /path/to/project --profile nextjs --install-gemini-md`.
