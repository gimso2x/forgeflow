...
```powershell
irm https://raw.githubusercontent.com/gimso2x/forgeflow/main/scripts/bootstrap_codex_plugin.py | python - --dry-run --force
...
.\scripts\run_orchestrator.ps1 init --task-id my-task-001 --objective "Update README quickstart" --risk small
```
...
1. make validate — artifact contract, schema 검증 (CLI 불필요)
2. make smoke-claude-plugin — plugin packaging, non-mutating guard 검증
3. exec-stage without --real — stub으로 JSON output 구조 확인
4. exec-stage --real — 실제 Claude/Codex CLI 호출, "execution_mode": "real" 확인 필수
