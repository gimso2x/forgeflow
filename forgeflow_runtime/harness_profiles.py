...
def _architect_agent(task_id: str, route: str, project_info: dict[str, Any]) -> str:
    framework = project_info.get("framework") or "TBD"
    language = project_info.get("language") or "TBD"
    return f"""\
---
name: architect
...
3. **기술 스택 선정**: 규모와 요구사항에 맞는 기술 + 선택 근거 (현재: {framework}, {language})
...
def _frontend_dev_agent(task_id: str, route: str, project_info: dict[str, Any]) -> str:
    framework = (project_info.get("framework") or "React").lower()
    language = project_info.get("language") or "TypeScript"
    
    if "next" in framework:
        init_steps = "Next.js 프로젝트 설정, App Router 구조 세팅"
        routing_info = "App Router 기반 페이지 구성, 동적 라우팅"
        dir_structure = """\
    src/
    ...
    ├── stores/
    └── types/"""
    elif "vite" in framework or "react" in framework:
        init_steps = f"{framework.capitalize()} 프로젝트 설정, Vite/빌드 도구 구성"
        routing_info = "React Router 또는 TanStack Router 기반 라우팅"
        dir_structure = """\
    src/
    ├── components/
    │   ├── ui/                 # 공통 UI 컴포넌트
    ...
    └── utils/                  # 유틸리티 함수"""
        ...
        init_steps = f"{framework} 환경 설정 및 구조 세팅"
        routing_info = f"{framework} 표준 라우팅 방식 적용"
        dir_structure = "표준 프로젝트 구조 준수"

    return f"""\
---
name: frontend-dev
description: "프론트엔드 개발자. {framework.capitalize()} UI 컴포넌트, 페이지 라우팅, 상태관리, API 연동, 반응형 디자인."
---
...
1. **프로젝트 초기화**: {init_steps}
2. **UI 컴포넌트 개발**: 재사용 컴포넌트, 스타일링 (Tailwind CSS 등)
3. **페이지/라우팅**: {routing_info}
4. **상태관리**: {language} 기반 상태 관리 (Zustand, React Query 등)
...
def _backend_dev_agent(task_id: str, route: str, project_info: dict[str, Any]) -> str:
    framework = project_info.get("framework") or "TBD"
    language = project_info.get("language") or "TBD"
    return f"""\
---
...
백엔드 개발 전문가. 안전하고 확장 가능한 서버 사이드 로직을 구현한다. (현재: {framework}, {language})
...
def _qa_engineer_agent(task_id: str, route: str, project_info: dict[str, Any]) -> str:
    framework = project_info.get("framework") or "TBD"
    return f"""\
---
name: qa-engineer
...
소프트웨어 품질 보증 전문가. 체계적인 테스트 전략으로 버그를 사전에 방지한다. ({framework} 환경)
...
def _devops_engineer_agent(task_id: str, route: str, project_info: dict[str, Any]) -> str:
    framework = project_info.get("framework") or "TBD"
    return f"""\
---
...
DevOps 전문가. 안정적이고 자동화된 배포 파이프라인을 구축한다. ({framework} 배포)
...
def _fullstack_webapp_skill(task_id: str, route: str, mode: str, project_info: dict[str, Any]) -> str:
    framework = project_info.get("framework") or "TBD"
    return f"""\
---
...
웹앱({framework})의 요구사항→설계→프론트엔드→백엔드→테스트→배포를 에이전트 팀이 협업하여 개발한다.
...
def _component_patterns_skill(task_id: str, project_info: dict[str, Any]) -> str:
    framework = project_info.get("framework") or "React"
    return f"""\
---
name: component-patterns
description: "{framework} 컴포넌트 설계 패턴. Compound/Render Props/HOC/Custom Hooks, 상태관리, 폴더 구조."
---

# Component Patterns — {framework} 컴포넌트 패턴
...
def _api_security_skill(task_id: str, project_info: dict[str, Any]) -> str:
    return f"""\
---
...
def resolve_profile(
    ...
    # Ensure project_info is at least an empty dict
    ...
    for name in agent_names:
        ...
        if gen:
            ...
            agents[path] = gen(task_id, route, p_info)
    ...
    for name in skill_names:
        ...
        if gen:
            if name == "fullstack-webapp":
                content = gen(task_id, route, mode, p_info)
            else:
                content = gen(task_id, p_info)
